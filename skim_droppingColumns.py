...
def dropColumns(columnList):
    ...
    blackListFile = open("/afs/cern.ch/work/m/mrogulji/X_YH_4b/columnBlackList.txt","r")
    badColumns = blackListFile.readlines()
    for badColumn in badColumns:
        if(badColumn in columnList):
            columnList.remove(badColumn)
...
opts.fMode = "RECREATE"
a.GetActiveNode().DataFrame.Snapshot("Events",options.output,dropColumns(a.DataFrame.GetColumnNames()),opts)
