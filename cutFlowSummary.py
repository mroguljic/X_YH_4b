...
parser.add_option('-c', '--channel', metavar='IFILE', type='string', action='store',
            default   =   'hadronic',
            dest      =   'channel',
            help      =   'Hadronic or semileptonic')
...
if(options.channel=="hadronic"):
    ...
    with open(options.json) as json_file:
        ...
        for sample, sample_cfg in data.items():
            ...
            for region in ["L","T"]:
                if("data_obs" in sample):
                    nCut = f.Get("{0}_mSD_{1}".format(sample,region)).Integral()
                else:
                    nCut = f.Get("{0}_mSD_{1}_nom".format(sample,region)).Integral()
                #nCut = nCut/(h.GetBinContent(1))
                if(nCut>10000):
                    latexRow = latexRow +" {0:1.2e} &".format(nCut)
                else:   
