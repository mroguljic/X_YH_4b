...
def create_jobs(config, dry=True, queue='',year="2016",jobs_dir="",out_dir="",nFiles=1,channel="mu"):
    for sample, sample_cfg in config.items():      
      ...
      for n, l  in enumerate(list(job_list)):
        ...
        argsFile.write("-i {0} -o {1} -p {2} -y {3} -c {4}\n".format(inputPath,outputPath,sample,year,channel))
      #Submit
...
def main():
  ...
  parser.add_argument('--channel',help='e or mu')
  ...
  with open(args.config, 'r') as config_file:
    ...
    create_jobs(config, args.dry,queue=args.queue, year=args.year,out_dir=args.outdir,jobs_dir=args.jobdir,nFiles=args.nFiles,channel=args.channel)
