...
def get2DTrigEff(hPass,hTot,outputFile,RebinX=1,RebinY=1,xTitle="",yTitle="",xLimits=[],yLimits=[],label=""):
    ...
    print(nBinsX)
    customMJYbins = np.array([60.,80.,100.,120.,140.,160.,200.,250.,320.,400.,500.,640.],dtype='float64')
...
with open(options.json) as json_file:
    ...
    for sample, sample_cfg in data.items():
        ...
        h2D_Tot             = f.Get("{0}_noTriggers_SR".format(sample))
        ...
        h2D_AllTrigers      = f.Get("{0}_triggersAll_SR".format(sample))
        ...
        get2DTrigEff(h2D_AllTrigers,h2D_Tot,"trig_eff_semires_{0}.png".format(year,sample),RebinX=10,RebinY=2,xTitle="M_{Jjj} [GeV]",yTitle="M_{jj} [GeV]",xLimits=[700,3000],yLimits=[60,640],label=sample_cfg["label"])

        eff_MJJ = getTrigEff(h_MJJ_AllTrigers,h_MJJ_Tot,"".format(sample),RebinX=10,xTitle="M_{Jjj}[GeV]",yTitle="Trigger efficiency",color=sample_cfg["color"])
        effs_MJJ.append(eff_MJJ)
        eff_MJY = getTrigEff(h_MJY_AllTrigers,h_MJY_Tot,"".format(sample),RebinX=2,xTitle="M_{jj}[GeV]",yTitle="Trigger efficiency",color=sample_cfg["color"])
        effs_MJY.append(eff_MJY)
        g = r.TFile.Open("TriggerEffs_semiResolved.root","UPDATE")
        g.cd()
...
plotTrigEffs(effs_MJJ,labels,"trig_eff_semires_MJJ_{0}.png".format(year),xLimits=[700,3000],yLimits=[0.5,1.1],legHeader="60 GeV < M_{JY} < 640 GeV")
plotTrigEffs(effs_MJY,labels,"trig_eff_semires_MJY_{0}.png".format(year),xLimits=[60,640],yLimits=[0.5,1.1],legHeader="700 GeV < M_{JJ} < 3000 GeV")
# plotTrigEffs(effs_MJJ,labels,"results/plots/{0}/trigger/all_MJJ_SR.pdf".format(year),xLimits=[700,3000],yLimits=[0.5,1.1],legHeader="60 GeV < M_{JY} < 320 GeV")
# plotTrigEffs(effs_MJY,labels,"results/plots/{0}/trigger/all_MJY_SR.pdf".format(year),xLimits=[60,320],yLimits=[0.5,1.1],legHeader="700 GeV < M_{JJ} < 3000 GeV")
